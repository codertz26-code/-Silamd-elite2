...
sila({ nomCom: 'goodbye',
    desc: 'Set goodbye message when members leave',
    Categorie: 'Group',
    reaction: '👋',
    fromMe: 'true',
    ...
    try {
        ...
        if (!arg || arg.length === 0) {
            const buttons = [
                { buttonId: `${prefixe}goodbye on`, buttonText: { displayText: '✅ Turn ON' }, type: 1 },
                { buttonId: `${prefixe}goodbye off`, buttonText: { displayText: '❌ Turn OFF' }, type: 1 }
            ];
...
This feature sends a goodbye message when members leave.

Choose an option:

✅ Turn ON - Say goodbye to leaving members
❌ Turn OFF - Disable goodbyes`,
                footer: "SILA-MD Goodbye © 2026",
                buttons: buttons,
                headerType: 4
            });
            return;
        ...
        repondre(`✅ Goodbye messages is now: *${next === 'yes' ? 'ON ✅' : 'OFF ❌'}*`);

    } catch (e) {
        console.log("❌ Goodbye Command Error: " + e);
